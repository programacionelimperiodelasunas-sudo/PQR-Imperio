	...
	async fetch(request, env, ctx) {
		...
		try {
			...
			if (path.startsWith("/api/users/") && method === "DELETE") {
				...
				const result = await env.pqr_d1.prepare("DELETE FROM users WHERE id = ?").bind(id).run();
			...
			if (path === "/api/registros" && method === "GET") {
				...
				} else if (rolParam === "cajero" || (rolParam && rolParam !== "administrador")) {
					// Cashiers only see records for nails, brows, and pedicure in their branch (sede)
					conditions.push("tipo_pqr IN ('Uñas Artificiales', 'Cejas y Pestañas', 'Pedicura Especializada')");
					if (sedeParam) {
						conditions.push("sede = ?");
						params.push(sedeParam);
			...
			if (path === "/api/registros" && method === "POST") {
				...
				// 1. Insert main registry first to obtain the exact registroId
				const insertRegResult = await env.pqr_d1.prepare(`
				...
				const registroId = insertRegResult.meta.last_row_id;
				...
				try {
					...
					if (data.condiciones_especiales) {
						...
						statements.push(
							...
							`).bind(
								...
								ce.especifique_condicion_piel || "",
								ce.posible_condicion_pestañas ? 1 : 0,
								ce.especifique_condicion_pestañas || ""
							)
						);
					}
					...
					if (statements.length > 0) {
						await env.pqr_d1.batch(statements);
					}
					...
					// Rollback by deleting the main registro on error
					await env.pqr_d1.prepare("DELETE FROM registros WHERE id = ?").bind(registroId).run();
					throw batchError;
				}
			}
